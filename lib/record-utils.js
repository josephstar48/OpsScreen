...
export function validateRecord(payload, isUpdate = false) {
  ...
  if (!sanitize(payload.orgId)) {
    throw new Error("Organization is required.");
  }

  if (!sanitize(payload.scenarioId)) {
    throw new Error("Scenario is required.");
  }

  if (!sanitize(payload.createdBy)) {
    throw new Error("Submitting user is required.");
  }
...
export function normalizeRecord(payload, isUpdate = false) {
  ...
  return {
    ...
    orgId: sanitize(payload.orgId),
    scenarioId: sanitize(payload.scenarioId),
    createdBy: sanitize(payload.createdBy),
    createdByName: sanitize(payload.createdByName),
    ...
    currentStatus: sanitize(payload.currentStatus) || "Awaiting processing",
