...
async function createSchema() {
  ...
  await addSchemaConstraints();
}

async function addSchemaConstraints() {
  await query(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_opsscreen_scenarios_org_scenario
      ON opsscreen_scenarios (org_id, scenario_id);
  `);

  await addForeignKey(
    "opsscreen_organizations",
    "fk_opsscreen_organizations_created_by",
    "FOREIGN KEY (created_by) REFERENCES opsscreen_users (user_id) ON DELETE SET NULL"
  );
  await addForeignKey(
    "opsscreen_org_memberships",
    "fk_opsscreen_org_memberships_org",
    "FOREIGN KEY (org_id) REFERENCES opsscreen_organizations (org_id) ON DELETE CASCADE"
  );
  await addForeignKey(
    "opsscreen_org_memberships",
    "fk_opsscreen_org_memberships_user",
    "FOREIGN KEY (user_id) REFERENCES opsscreen_users (user_id) ON DELETE CASCADE"
  );
  await addForeignKey(
    "opsscreen_scenarios",
    "fk_opsscreen_scenarios_org",
    "FOREIGN KEY (org_id) REFERENCES opsscreen_organizations (org_id) ON DELETE CASCADE"
  );
  await addForeignKey(
    "opsscreen_scenarios",
    "fk_opsscreen_scenarios_created_by",
    "FOREIGN KEY (created_by) REFERENCES opsscreen_users (user_id) ON DELETE RESTRICT"
  );
  await addForeignKey(
    "opsscreen_scenario_memberships",
    "fk_opsscreen_scenario_memberships_scenario",
    "FOREIGN KEY (scenario_id) REFERENCES opsscreen_scenarios (scenario_id) ON DELETE CASCADE"
  );
  await addForeignKey(
    "opsscreen_scenario_memberships",
    "fk_opsscreen_scenario_memberships_user",
    "FOREIGN KEY (user_id) REFERENCES opsscreen_users (user_id) ON DELETE CASCADE"
  );
  await addForeignKey(
    "opsscreen_records",
    "fk_opsscreen_records_org",
    "FOREIGN KEY (org_id) REFERENCES opsscreen_organizations (org_id) ON DELETE RESTRICT"
  );
  await addForeignKey(
    "opsscreen_records",
    "fk_opsscreen_records_scenario",
    "FOREIGN KEY (scenario_id) REFERENCES opsscreen_scenarios (scenario_id) ON DELETE RESTRICT"
  );
  await addForeignKey(
    "opsscreen_records",
    "fk_opsscreen_records_org_scenario",
    "FOREIGN KEY (org_id, scenario_id) REFERENCES opsscreen_scenarios (org_id, scenario_id) ON DELETE RESTRICT"
  );
  await addForeignKey(
    "opsscreen_records",
    "fk_opsscreen_records_created_by",
    "FOREIGN KEY (created_by) REFERENCES opsscreen_users (user_id) ON DELETE RESTRICT"
  );
  await addForeignKey(
    "opsscreen_audit_log",
    "fk_opsscreen_audit_log_org",
    "FOREIGN KEY (org_id) REFERENCES opsscreen_organizations (org_id) ON DELETE SET NULL"
  );
  await addForeignKey(
    "opsscreen_audit_log",
    "fk_opsscreen_audit_log_actor",
    "FOREIGN KEY (actor) REFERENCES opsscreen_users (user_id) ON DELETE RESTRICT"
  );
}
